...
function simulate(Atmosphere::Module, max_scatterings::Real, tau_max::Real, total_packets::Real, rng::MersenneTwister)
    ...
    Threads.@thread for box in ProgressBar(1:total_boxes)
        ...
        # Add to local field  
        Threads.atomic_add!(J[i,j,k], packets)
        ...
        for packet=1:packets
            ...
            for s=1:Int(max_scatterings)
                ...
                # Scatter packet once 
                r, box_id, escape, escape_angle, destroyed, J = scatter_packet!(x, y, z, chi, edge, box_id, r, J, boundary, rng)

                # Check if escaped
                if escape
                    ϕ, θ  = escape_angle 
                    Threads.atomic_add!(surface[box_id[1], box_id[2], 1 + Int(ϕ÷(π/2)), 1 + Int(θ>(π/8))], 1)
